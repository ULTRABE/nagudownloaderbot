...
from utils.media_processor import reencode_shorts, ensure_fits_telegram, extract_audio_from_video
...
def _layer2_opts(tmp: Path, format_str: str) -> dict:
    """Layer 2: Alternative extractor config"""
    opts = _base_opts(tmp)
    opts["format"] = format_str
...
async def _try_download(url: str, opts: dict) -> Optional[Path]:
    ...
    """
    # Extract directory from outtmpl (e.g. "/tmp/xxx/%(title)s.%(ext)s" → "/tmp/xxx")
...
async def download_youtube_video(url: str, tmp: Path, format_str: str = "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best") -> Optional[Path]:
    """
...
async def download_youtube_audio(url: str, tmp: Path) -> Optional[Path]:
    """
    Download YouTube audio (best quality MP3) using layered extraction.
    """
...
# Maps callback_data -> (video_path, audio_path, tmp_dir_context)
_pending: dict = {}

# ─── Shorts handler ───────────────────────────────────────────────────────────

async def handle_youtube_short(m: Message, url: str):
    """Handle YouTube Shorts — download, re-encode, send"""
    ...
    try:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            # Re-encode for quality + smaller size
            encoded = tmp / f"short_encoded.mp4"
...
async def handle_youtube_normal(m: Message, url: str):
    ...
    """
    # Show inline buttons immediately
